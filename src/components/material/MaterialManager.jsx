...
function ImageModelBadge({ filePath, fileName, variant = 'grid' }) {
...
export default function MaterialManager() {
  ...
  // 获取图片尺寸的函数
  const getImageDimensions = useCallback((imagePath) => {
  ...
  const loadFolder = useCallback(
    async (path = '') => {
      ...
      setError(null)
      try {
        const result = await browseFolderApi(path)
  ...
  const loadFolderContents = useCallback(async (folderPath) => {
    try {
      const result = await browseFolderApi(folderPath)
  ...
  const handleAddToChat = useCallback((file, event) => {
    if (event) {
  ...
  const getFileIcon = useCallback((type, className = 'w-4 h-4') => {
    switch (type) {
      case 'folder':
