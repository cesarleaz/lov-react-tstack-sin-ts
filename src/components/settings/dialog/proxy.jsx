...
const SettingProxy = () => {
  ...
  useEffect(() => {
    const loadConfig = async () => {
      try {
        ...
        const proxyValue = settings.proxy || 'system'
        let mode = 'system'
        let url = ''
  ...
  const handleSave = async () => {
    try {
      ...
      switch (proxyConfig.mode) {
        ...
        case 'custom':
          ...
          if (proxyValue && !proxyValue.match(/^(https?|socks[45]):\/\/.+/)) {
            setErrorMessage(
              'Invalid proxy URL format. Please use http://, https://, socks4://, or socks5:// protocol.'
            )
            return
          }
          break
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Failed to save proxy settings'
      setErrorMessage(errorMessage)
              ...
              <Select value={proxyConfig.mode} onValueChange={handleModeChange}>
                ...
                <SelectContent>
                  <SelectItem value="no_proxy">
                    {t('settings:proxy:modes.no_proxy')}
                  </SelectItem>
                  <SelectItem value="system">
                    {t('settings:proxy:modes.system')}
                  </SelectItem>
                  <SelectItem value="custom">
                    {t('settings:proxy:modes.custom')}
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
